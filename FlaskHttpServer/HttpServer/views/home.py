from flask import Blueprint, render_template, current_app, send_from_directory, request, jsonify,Flask
import os
import time
...
def upload():
    if request.method == 'GET':
        ...
        base_dir = current_app.config['BASEDIR']
        load_path = os.path.join(base_dir, path, file_name)
        if os.access(load_path, os.F_OK):
            return jsonify({"code": 200, "info": "文件：%s 已存在" % file_name})
        else:
            file.save(load_path)
